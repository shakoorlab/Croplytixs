...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      port: PORT,
      host: true,
      // Same-origin in dev too: the page calls `/api/...` and Vite forwards it to
      // the backend, so no CORS and no API URL baked into the bundle. Point
      // VITE_API_PROXY at the Cloud Run URL to develop against the real service.
      proxy: {
        '/api': {
